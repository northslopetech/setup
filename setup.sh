...
function get_latest_version {
    if [[ -z "${CACHED_LATEST_VERSION}" ]]; then
        CACHED_LATEST_VERSION=$(curl -sL https://api.github.com/repos/northslopetech/setup/releases/latest | grep tag_name | awk -F'"' '{ print $4 }')
    fi
